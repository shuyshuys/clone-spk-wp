...
@section('content')
    ...
    <div class="row my-4">
        ...
        <div class="col-xl-4 col-md-6">
            <a
                href="{{ route('assessments.bestEmployee', ['month' => request()->input('month'), 'year' => request()->input('year')]) }}">
                <button type="button" class="btn btn-success">
                    <span class="ps-2 fw-bolder">Lihat Project Management Terbaik</span>
                </button>
            </a>
        </div>
    </div>
            ...
            <div class="col-lg-4">
                <select class="form-select" name="month">
                    @foreach (range(1, 12) as $month)
                        <option value="{{ $month }}" @if ($month == request('month', date('n'))) selected @endif>
                ...
                <div class="input-group">
                    <select class="form-select" name="year">
                        @foreach (range(date('Y'), 2020) as $year)
                            <option value="{{ $year }}" @if ($year == request('year', date('Y'))) selected @endif>
                                {{ $year }}</option>
                        @endforeach
                    </select>
        ...
        <div class="tabel-adm col-xl-3 col-md-6 mx-auto">
            ...
            <div class="table-responsive w-auto">
                <table class="table table-bordered">
                    <thead>
                        <th scope="col">#</th>
                        <th scope="col">Nama Project Management</th>
                        ...
                        @foreach ($groupedAssessments as $uuid => $data)
                            <tr>
                                ...
                                @can('update', $assessment)
                                    <td class="d-flex">
                                        <a href="{{ route('assessments.edit', $uuid) }}"><button type="button"
