        ...
        <div class="add-admin col-12 bg-white">
            <form id="save-assessment-form" action="{{ route('assessments.store') }}" class="pt-5 px-md-3" method="post">
                ...
                <div class="form-group row mb-4 px-3">
                    <label for="user_uuid" class="form-label"> </label>
                ...
                @foreach ($criterias as $criteria)
                    ...
                    <div class="form-group row mb-4 px-3">
                        ...
                        <input id="{{ $inputName }}" name="{{ $inputName }}" type="number" class="form-control"
                            placeholder="Masukkan Nilai {{ $criteria->name }}" required>
                    </div>
                @endforeach
                <div class="form-group row mb-4 px-3">
                    ...
                    <select class="form-select" name="month" id="month">
                        @foreach (range(1, 12) as $month)
                            <option value="{{ $month }}" @if ($month == date('n')) selected @endif>
